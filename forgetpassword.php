...
function sendMail($email,$reset_token)
{
   ...
   try {
      ...
      $mail->Password   = 'clzv sftg gsor dzpc';                               //SMTP password
      ...
      <a href='http://localhost/FA/updatepassword.php?email=$email&reset_token=$reset_token' >
      Reset Password 
      </a>";
...
{
 $query = "SELECT * FROM `fa` WHERE `email`='$_POST[email]'";
 ...
 if($result)
 {
  if(mysqli_num_rows($result)==1)
  {
    ...
    $date =date("Y-m-d");
    $query = "UPDATE `fa` SET `resettoken`='$reset_token',`resettokenexpire`='$date' WHERE `email`='$_POST[email]'";
